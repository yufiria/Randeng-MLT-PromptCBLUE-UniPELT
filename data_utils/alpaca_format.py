"""
文件功能说明：
本文件用于将PromptCBLUE数据集转换为Alpaca指令微调格式。

主要功能：
1. 定义各个医疗NLP任务的指令模板（TASK_TO_INSTRUCTION）
2. 提供数据集按任务类型拆分的功能（split_task）
3. 将原始数据转换为Alpaca格式（alpaca_format）
...
- sample_id: 样本ID
- task_dataset: 任务类型
- answer_choices: 答案选项（如果有）

使用方式：
    调用alpaca_format函数，指定数据路径，将生成alpaca格式的JSON文件
"""
...
def split_task(path, dim):
    ...
    参数：
        path: 数据集文件夹路径
        dim: 数据集维度（train/validation/test）
    
    功能：
        将混合任务的数据集按task_dataset字段拆分为单独的任务文件
        保存到 data/{task_name}/dev.json
    """
    # Load dataset
...
def alpaca_format(path):
    ...
    参数：
        path: 数据文件路径（包含train.json, dev.json, test.json）
    
    功能：
        1. 读取原始JSON格式的数据
        2. 为每个样本添加instruction字段（从TASK_TO_INSTRUCTION获取）
        3. 重组为Alpaca格式的字典
        4. 保存为alpaca_{split}.json文件
