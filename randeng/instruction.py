"""
文件功能说明：
本文件是PromptCBLUE医疗数据集的任务指令配置文件。
定义了各个医疗NLP任务的指令模板、最大生成token数和任务类型映射。

主要内容：
1. TASK_TO_INSTRUCTION: 任务名到任务指令的映射字典
2. TASK_TO_MAX_NEW_TOKENS: 任务名到最大生成token数的映射
3. TASK_TO_TASK_TYPE: 任务名到任务类型的映射
